...
export const ko: Messages = {
  ...
  modes: {
    ...
    catalog: {
      classic: {
        name: '클래식',
        blurb: '도시를 건너뛰며 누적 거리를 목표 거리에 가깝게 맞추세요.',
      },
      hidden: {
        name: '숨겨진 목적지',
  ...
  about: {
    ...
    rules:
      '가능한 한 적은 횟수로 목표에 충분히 가까이 도달하세요. 기회는 6번이며, 목표를 넘어도 라운드가 끝나지 않습니다. 메뉴에서 모드를 고르면 연속 기록에 영향을 주지 않는 무작위 문제를 무제한으로 즐길 수 있습니다.',
    credits:
  ...
  prompt: {
    ...
    targetLabel: '총합 목표',
    hint: (band, guesses) =>
      `도시에서 도시로 이동 · 목표에서 ${band} 이내로 도달 · ${guesses}번의 기회`,
  ...
  howTo: {
    title: '게임 방법',
    intro:
      '매일 출발 도시 하나와 목표 거리 하나. 도시를 하나씩 이어 경로를 만들고 각 구간을 더하세요 — 합계를 목표에 충분히 가깝게 맞추세요.',
    step1:
      '도시를 추측하세요. 점수는 출발지에서 그 도시까지의 직선 거리입니다. 다시 추측하면 마지막 도시에서 새 도시까지의 구간이 더해집니다.',
    step2:
      '지구본의 점선 원이 남은 거리를 그대로 보여줍니다 — 그 위에 있는 도시를 노리세요. 합계가 가까워질수록 뜨거움/차가움 신호가 따뜻해집니다.',
    step3: (band, guesses) =>
      `합계를 목표에서 ${band} 이내로 — 어느 쪽이든 — 맞추면 승리입니다. 목표를 넘어도 잃는 것은 그 한 번뿐이라 파산은 없습니다. ${guesses}번의 기회를 모두 써야만 패배합니다. 구간이 적을수록 좋은 점수입니다.`,
    note: (min) =>
