...
export const zh: Messages = {
  ...
  about: {
    ...
    rules:
      '用尽量少的跳数，让总数足够接近目标。你有 6 次机会，超过目标也不会结束本局。从菜单中选择模式，即可无限畅玩不计入连胜的随机题目。',
  ...
  howTo: {
    title: '玩法',
    intro:
      '每天一座出发城市和一个目标距离。逐城构建路线并累加每一跳——让总数足够接近目标。',
    step1:
      '猜一座城市。你的得分是从出发地到它的直线距离。再猜一座，从你上一座城市到新城市的这一跳会累加上去。',
    step2:
      '地球仪上的虚线圆环正好标出你还需跳多远——瞄准落在圆环上的城市。随着累计总数接近目标，冷热提示会逐渐变热。',
    step3: (band, guesses) =>
      `让总数落在目标前后 ${band} 以内即可获胜。超过目标只损失这一次机会，不会爆掉。只有用完 ${guesses} 次机会才会输。跳数越少，得分越好。`,
    note: (min) =>
