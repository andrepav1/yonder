...
**Globe** (`--globe-ocean/-graticule/-coast/-border` + the `--hypso-0…9` ramp): a muted,
...
coastlines crisp. **Country borders** are a fainter, thinner `--globe-border` hairline
inland of the coast — deliberately quieter than the coastline (0.3 width at 55%
opacity) so the political layer reads as context under the pins, never as a competing
line. Kept
